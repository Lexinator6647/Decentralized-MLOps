...
class BlockchainMetricsWrapper:
    ...
    def __init__(self, ml_step: str, output_dir: str = "."):
        ...
        # point at the JS updater in ML_Verse
        base              = os.path.dirname(__file__)
        self.js_update_dir = os.path.join(base, "ML_Verse")
    ...
    def send_to_blockchain(self):
        if not os.path.isfile(self.js_update_path):
            print(f"[{self.ml_step}] No script at {self.js_update_path}, skipping blockchain update.")
            return

        # Run the JS script *from* ML_Verse so it can read train_metrics.json
        try:
            result = subprocess.run(
                ["node", os.path.basename(self.js_update_path)],
